...
public class AccountService : IAccountService
{
    ...
    public async Task<bool> SyncAcountAsync(AccountStateEventData accountEvent, string syncType)
    {
        ...
        string[] syncTypesThatNeedsAnExistingAccount = { OperationAction.Update, OperationAction.Delete };
        ...
        if (account is not null && syncType.Equals(OperationAction.Insert))
        {
    ...
    public async Task UpdateAccountProcessStatusAsync(string accountNumber, string operationName)
    {
        ...
        var operations = await this.operationRepository.FetchOperationsByCriteriaAsync(
                                   criteria,
                                   OperationStrategyType.ACCOUNT,
                                   accountNumber);

        await this.operationRepository.BulkUpdateOperationsStatusAsync(ProcessStatus.Succeeded.ToString(), operations);
    }
